...
| B2. Reorder / priority | **Done** | `QueueJob` gains optional `priority` (default 0); `JobQueue.addJob(..., priority)` stores it and `processNext` picks the max-priority QUEUED job, then FIFO. Reordering is drag-and-drop via @dnd-kit: `JobQueue.moveJobTo(id, toPosition)` permutes the QUEUED subsequence (non-queued jobs keep their slots, target clamped), emits `'moved'` `{id, toPosition}`. New IPC `IPC.QUEUE_MOVE_TO` (`queue-move-to`) → `queue.ts` handler + `moved` event forwarded as `IPC.QUEUE_MOVED` (`queue-moved`); preload `queueMoveTo()`/`onQueueMoved()` + d.ts + stub. `QueueJobCard` is a dnd-kit sortable item with a `faGripVertical` drag handle (`batchQueue.dragHandle`) on QUEUED jobs; `BatchQueue` wraps the list in `DndContext`/`SortableContext`, shows a `DragOverlay` clone, derives the drop target via `computeQueuedTargetPosition` (utils/queue-reorder.ts), and mirrors `onQueueMoved` via `reorderJob`. Tests: job-queue (priority + moveJobTo), queue handlers, preload index, ipc-channels, QueueJobCard, BatchQueue, queue-reorder helper. See docs/DND_REORDER_PLAN.md. |

# IPC-Kanäle & electronAPI

Alle Kanalnamen sind in `src/shared/ipc-channels.ts` zentralisiert.

## Requests (invoke aus dem Renderer)
...
Das Preload-Skript stellt den gesamten IPC über `window.electronAPI` bereit (typisiert in `src/renderer/electron-api.d.ts`):
...
### Event-Listener (jeder gibt eine Cleanup-Funktion zurück)

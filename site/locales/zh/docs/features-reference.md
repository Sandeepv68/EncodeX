...
将视频和图片顺时针旋转 90°、180° 或 270°，并支持水平或垂直镜像——在转换页面和批量队列中均可完成（转码与图片压缩面板，以及单个任务的选项对话框）。旋转使用 FFmpeg 的 `transpose` 滤镜（`transpose=1` / `transpose=2,transpose=2` / `transpose=2`），与 `hflip`/`vflip` 一起合并到 `scale` 旁的单个 `-vf` 滤镜链中；镜像始终需要重新编码。在流复制模式下，旋转仅对 MP4/MOV/MKV 输出以无损的 `-metadata:s:v rotate=N` 保存；其他容器和图片输出始终走像素（重新编码）路径。

### 转换配置文件

应用预配置的编码预设以立即填充转换设置。配置文件封装了完整的编码配置——容器格式、视频编解码器、音频编解码器、比特率、CRF/质量、缩放、像素格式和高级 FFmpeg 参数——这样你就不必手动配置每一项设置。

- **8 个类别中的 140+ 内置配置文件**：Web 与社交（YouTube、Instagram、TikTok、Facebook、X）、设备（Apple、Android、游戏主机）、视频编解码器、专业（ProRes、DNxHD/HR、FFV1）、流媒体（HLS、DASH）、音频、图片和高级
- **自定义配置文件**——创建、编辑和删除你自己的配置文件；保存在本地存储中
- **最近使用**——快速访问最近应用的 5 个配置文件
- **类别筛选**——通过带图标徽章的类别浏览配置文件
- **批量支持**——将配置文件应用于单个批处理任务或整个队列

配置文件在转换页面和批处理队列中均可用。选择配置文件会自动填充所有相关的编码字段，同时仍允许手动覆盖。

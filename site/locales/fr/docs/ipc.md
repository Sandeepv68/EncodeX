# Canaux IPC & electronAPI

Tous les noms de canaux sont centralisés dans `src/shared/ipc-channels.ts`.

## Requêtes (invoke depuis le renderer)
...
Le script preload expose tout l'IPC via `window.electronAPI` (typé dans `src/renderer/electron-api.d.ts`) :
...
### Listeners d'événements (chacun renvoie une fonction de nettoyage)

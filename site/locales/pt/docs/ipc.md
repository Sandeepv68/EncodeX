# Canais IPC & electronAPI

Todos os nomes de canais são centralizados em `src/shared/ipc-channels.ts`.

## Requisições (invoke do renderer)
...
## Somente envio (renderer para main)
...
O script preload expõe todo o IPC via `window.electronAPI` (tipada em `src/renderer/electron-api.d.ts`):
...
### Listeners de eventos (cada um retorna uma função de limpeza)
